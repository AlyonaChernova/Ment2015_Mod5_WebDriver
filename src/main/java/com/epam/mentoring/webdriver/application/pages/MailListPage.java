...
public class MailListPage extends AbstractPage {
    ...
    public boolean checkMessageContent(Mail mail, boolean isDrafts) {
        ...
        if (mailLine != null) {
            mailLine.click();
            if (isDrafts) {
                Browser.getInstance().switchTo().frame(FindIFrameForText.getMailBodyFrame(Browser.getInstance()));
    ...
    public boolean moveToSpam(Mail mail) {
        WebElementActions.dragAndDrop(getMailRow(mail), lnkSpamBin);
    ...
    public boolean deleteFromSpam(Mail mail) {
        ...
        if (mailRow != null) {
            ...
            WebElementActions.clickByAction(checkBoxMailRow);
            WebElementActions.sendKeyByAction(Keys.DELETE);
